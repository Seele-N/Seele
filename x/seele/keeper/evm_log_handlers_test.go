...
func (suite *KeeperTestSuite) TestSendToAccountHandler() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			handler := keeper.NewSendToAccountHandler(suite.app.BankKeeper, suite.app.SeeleKeeper)
			tc.malleate()
...
func (suite *KeeperTestSuite) TestSendToEthereumHandler() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			func() {
				suite.app.SeeleKeeper.SetExternalContractForDenom(suite.ctx, validDenom, contract)
			...
			func() {},
			errors.New("native action __SeeleSendToEthereum is not implemented"),
		},
	}
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			suite.SetupTest()
			handler := keeper.NewSendToEthereumHandler(suite.app.SeeleKeeper)
			tc.malleate()
			err := handler.Handle(suite.ctx, contract, data)
			if tc.error != nil {
				suite.Require().EqualError(err, tc.error.Error())
...
func (suite *KeeperTestSuite) TestSendToIbcHandler() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				suite.app.SeeleKeeper.SetExternalContractForDenom(suite.ctx, invalidDenom, contract)
				coin := sdk.NewCoin(invalidDenom, sdk.NewInt(100))
				err := suite.MintCoins(sdk.AccAddress(contract.Bytes()), sdk.NewCoins(coin))
				suite.Require().NoError(err)
