...
func (suite *SeeleTestSuite) TestInitGenesis() {
	testCases := []struct {
		name     string
		malleate func()
		...
		suite.Run(tc.name, func() {
			...
			if tc.expPanic {
				suite.Require().Panics(
					func() {
						seele.InitGenesis(suite.ctx, suite.app.SeeleKeeper, *tc.genState)
					},
				)
			} else {
...
func (suite *SeeleTestSuite) TestExportGenesis() {
	genesisState := seele.ExportGenesis(suite.ctx, suite.app.SeeleKeeper)
